    ...
    csv.parse(data, {delimiter: ';', skip_empty_lines: true, trim: true, auto_parse: false, columns: true}, function(err, data) {
      ...
      var tokenScore = function tokenScore(wordlist, token, weight) {
        ...
        if (wordlist.hasOwnProperty(token)) {
          ...
          return {value: match.EMO_MEAN * weight, assessment: match};
        }
        return null;
      };

      var normalizeScore = function normalizeScore(metadata, score) {
        if (score === null || score === undefined || score == 0.0) return score;
        var oldscore = score;
        if (score > 0) {
          score = score * (1.0 / metadata.emo_max);
        } else if (score < 0) {
          score = (score - metadata.emo_min) * (1.0 / (-metadata.emo_min)) + -1.0;
        }
      ...
      cb(null, dict, metadata, tokenScore, normalizeScore);
    });
  });
